...
public class BarcodeManager {
	...
	public final static int MODEL_EM3396 = 5;
	public final static int MODEL_EM3296 = 6;
	public final static int MODEL_TOT2D03=7;
	// 扫描头协议内容
	...
	public final static int Protocol_EM3296_Default =11;// EM3296还原出厂设置
	public final static int Protocol_EM3296_Protocol =12; // 设置EM3296受保护的协议

	public final static int Protocol_EM3396_Default =13;// EM3396还原出厂设置
	public final static int Protocol_EM3396_Protocol =14; // 设置EM3396受保护的协议

	public final static int Protocol_TOT2D03_Default =15;// TOT2D03还原出厂设置
	public final static int Protocol_TOT2D03_Protocol =16; // 设置TOT2D03受保护的协议
	...
	// 30 06 2e
	private final byte[] N4313Prefix_Aim_id = new byte[] { 0x16, 0x4D, 0x0D,
			(byte) 0x70, (byte) 0x72, (byte) 0x65, (byte) 0x62, (byte) 0x6b,
			(byte) 0x32, (byte) 0x39, (byte) 0x39, (byte) 0x35, (byte) 0x63,
			(byte) 0x38, (byte) 0x31, (byte) 0x2e }; // 添加前缀CODEID 
	...
	private byte[] tot_codeid = {0x07, (byte)0xC6, 0x04, 0x08, 0x00, 0x2D, 0x02, (byte)0xFE, (byte)0xF8};
	 ...
	 */
	public int autoChoiseScanPrefix() {
		...
		case MODEL_EM3396:
			MODELS = "EM3396";
			if (getScanIsReturnFactory()) {
				prefix = Protocol_EM3396_Default;
			} else {
				prefix = Protocol_EM3396_Protocol;
			}
			break;
		case MODEL_EM3296:
			MODELS = "EM3296";
			if (getScanIsReturnFactory()) {
				prefix = Protocol_EM3296_Default;
			} else {
				prefix = Protocol_EM3296_Protocol;
			}
			break;
		case MODEL_EM3095:
			MODELS = "EM3095";
		...
		case MODEL_TOT2D03:
			MODELS = "TOT2D03";
			if (getScanIsReturnFactory()) {
				prefix = Protocol_TOT2D03_Default;
			} else {
				prefix = Protocol_TOT2D03_Protocol;
	 ...
	 */
	public void setScannerModel(int model) {
		//Log.i(TAG, "Scan_model:" + model,new Exception());
	...
	public void setScanPrefix() {
		int prefix = autoChoiseScanPrefix();
		//int prefix = Protocol_N4313_Default;
		...
		case Protocol_EM3296_Default:
			setScannerModel(MODEL_EM3296);
			break;
		case Protocol_EM3296_Protocol:
			setScannerModel(MODEL_EM3296);
			break;
		
		case Protocol_EM3396_Default:
			setScannerModel(MODEL_EM3396);
			break;
			
		case Protocol_EM3396_Protocol:
			setScannerModel(MODEL_EM3396);
			break;
			
		case Protocol_TOT2D03_Protocol:
			setScannerModel(MODEL_TOT2D03);
			break;
			
		case Protocol_TOT2D03_Default:
			setScannerModel(MODEL_TOT2D03);
			break;
	...
	public void Barcode_Open(Context context, Callback dataReceive) {
		...
		if(exist("/dev/ttySAC3")){
			Log.v(TAG,"Barcode_Open  device_type834: "+device_type);
			device_type = 1;//380A-Qualcom
		}else if(exist("/dev/ttyMT1")){
			Log.v(TAG,"Barcode_Open  device_type837: "+device_type);
			device_type = 2;//380K-MTK
		}else if(exist("/dev/ttyHSL1")){
			device_type = 3;//380K-Qualcom
		}
		Log.v(TAG,"Barcode_Open  device_type: "+device_type);
		...
		if (device_type == 3) {
			...
			serialPort_Path = "/dev/ttyMT1";
			setScannerModel(getScannerModel());
			//setScannerModel(MODEL_N4313);//
			//setScannerModel(MODEL_N3680);//N3680
			trigFile = new File("/proc/jbcommon/gpio_control/scan_switch");// 写"0"出光，写“1”关闭
			power = new File("/proc/jbcommon/gpio_control/ir_en");
			openCom = new File("/proc/jbcommon/gpio_control/scan_reset");
			if (!trigFile.canWrite()) {
				System.out.println("m9_dev.69 can not write");
				trigFile = new File("/proc/jbcommon/gpio_control/scan_switch");// 写"0"出光，写“1”关闭
				power = new File("/proc/jbcommon/gpio_control/scan_en");
			}
			trig_scan = "0";
		} else {
	...
	public void setScanIsReturnFactory(boolean b) {
		Log.e("jiebao", "setScanIsReturnFactory");
	...
	private synchronized void scan(String trig) {
		...
		Log.d(TAG, "scan()  scan_curTime: "+scan_curTime+" scan_preTime: "+scan_preTime);
		if (scan_curTime - scan_preTime < 120) {
			if (!trig_last.equals(trig)) {
				try {
					if (trig_last.equals(trig_scan)) {
						Thread.sleep(120);
						Log.d(TAG, "scan() sleep(120) " + "trig:" + trig+"scan_curTime: "+scan_curTime);
					}
					doScan(trig);
				} catch (InterruptedException e) {
					e.printStackTrace();
...
//						////Log.e("trig", "scan:" + all_scan_count);
		...
		Log.e(TAG, "trig:" + trig+" scan_preTime: "+scan_preTime);
	}
						...
						if (size > 0) {
							if (onDataListener != null) {
								...
								isCutData = false;
									...
									mHandler.postAtTime(sendBuffer,
											SystemClock.uptimeMillis() + 19);//59
								}
								cout = 0;
							}
	   ...
	   Log.v(TAG, "init() type: "+type);
		try {
			...
			case Protocol_N3680_Codeid:
				setIsReceive(true);
				notifyReader();
				//writeCommand(N4313Prefix_default);//恢复出厂设置	,防止出现误码		
				writeCommand(wakeUp);
				try {
					Thread.sleep(100);//恢复出厂设置,防止出现误码
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	private void dealData(byte[] buffer, int size) {
		try {
			...
			if (getScannerModel() == MODEL_N4313
				...
				if (str.equals("505245424b32393935633830062e")||str.equals("505245424b32393935633830062e505245424b32393935633830062e")) {
					Log.d(TAG, "init_N4313_Code_id Codeid success!");
				...
				if (str.equals("535546424b32393931323134062e")||str.equals("535546424b32393931323134062e535546424b32393931323134062e")) {
					Log.d(TAG, "init_N4313_end end success!");
					init_N4313_end = true;
				...
				// 444546414c54062e
				if (str.equals("444546414c54062e")||str.equals("444546414c54062e444546414c54062e")) {
	...
	private void getNewLand_Em3095(byte[] buffer) {
		// 3095
		Log.e(TAG, "getNewLand_Em3095:" + Tools.bytesToHexString(buffer)+" serialPortData_buffer: "+Tools.bytesToHexString(serialPortData_buffer));
		if (null == serialPortData_buffer) {
			if (buffer.length >= 2 && buffer[0] <= 35
				...
				// Tools.getNowTimeHm());
			} else if(buffer[buffer.length - 1] == (byte) 0x0d){
				getDefault(buffer);
			}else {
				// 错误数据 ， 或者协议可能没写入
		...
		if (null != buffer) {
			if (null == serialPortData_buffer) {
				if (buffer.length > 2 && isN4313Code_head(buffer[0])
						&& ((buffer[buffer.length - 1] == (byte) 0x14 && buffer[buffer.length - 2] == (byte) 0x12)||
							(buffer[buffer.length - 1] == (byte) 0x0A && buffer[buffer.length - 2] == (byte) 0x0D))) {
					...
					SerialPortData_head(buffer);
					System.out.println("SerialPortData_head");
				} else {
					// 错误数据 ， 或者协议可能没写入
	...
	private void SerialPortData_total_4313_CodeId(byte[] portData) {
		...
		for (int i = 0; i < list_byte.size(); i++) {
			if (isN4313Code_head(list_byte.get(i)[0])
					&& ((list_byte.get(i)[list_byte.get(i).length - 1] == (byte) 0x14&& list_byte.get(i)[list_byte.get(i).length - 2] == (byte) 0x12)||
					(list_byte.get(i)[list_byte.get(i).length - 1] == (byte) 0x0A&& list_byte.get(i)[list_byte.get(i).length - 2] == (byte) 0x0D))) {
	...
	private ArrayList<byte[]> checkSerialPortData_N4313_CodeId(
		...
		for (int i = 0; i < serialPortData.length; i++) {
			...
			if (isHasHead) {
				if (i > 0 && ((serialPortData[i - 1] == 0x12 && serialPortData[i] == 0x14)||
						(serialPortData[i - 1] == (byte) 0x0D && serialPortData[i - 1] == (byte) 0x0A))) {
					iEnd = i;
