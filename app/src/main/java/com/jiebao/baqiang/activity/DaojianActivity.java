...
public class DaojianActivity extends BaseActivityWithTitleAndNumber implements View
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        switch (keyCode) {
            case Constant.SCAN_KEY_CODE: {
                if (!SalesServiceDBHelper.checkServerInfo(mTvPreviousStation.getText().toString()
                )) {
                    Toast.makeText(DaojianActivity.this, "上一站网点信息异常", Toast.LENGTH_SHORT).show();
    ...
    public void clickHappend(int position) {
        ...
        CargoArrivalFileContent barcode = (CargoArrivalFileContent) mListData.get(position)
                .getScannerBean();
        // 此处ListView中数据是有ID值的
        if (DaojianDBHelper.isRecordUpload(barcode.getId())) {
            Toast.makeText(DaojianActivity.this, "当前记录已上传，不能删除", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void prepareDataForView() {
        ...
        mPreviousStationAdapter = new TestTipsAdatper(DaojianActivity.this, mPreviousStationInfo,
                new FilterListener() {
        ...
        mCargoArrivalFileContent = FileContentHelper.getCargoArrivalFileContent();

        mListData = new ArrayList<>();
        mScannerBaseAdatper = new CommonScannerBaseAdapter(DaojianActivity.this, mListData);
    }
    ...
    private List<String> resolvePreviousStationData() {
        ...
        return isOpen ? SalesServiceDBHelper.getSalesServiceOfCentreOrBranch(2) :
    ...
    private boolean insertForScanner(String barcode) {
        ...
        // 该结果从 扫码时间 转化得来
        mCargoArrivalFileContent.setOperateDate(new SimpleDateFormat("yyyMMdd").format(scanDate));
    ...
    private void updateUIForScanner(String barcode) {
        ...
        CargoArrivalFileContent bean = DaojianDBHelper.getNewInRecord(mCargoArrivalFileContent
                .getShipmentNumber(), mCargoArrivalFileContent.getScanDate());
    ...
    private void increaseOrDecreaseRecords(int actionType) {
        int unloadRecords = SharedUtil.getInt(DaojianActivity.this, Constant.PREFERENCE_NAME_DJ);
        ...
        SharedUtil.putInt(DaojianActivity.this, Constant.PREFERENCE_NAME_DJ, unloadRecords);
     ...
     * @param bean
     */
    private void deleteChooseRecord(final CargoArrivalFileContent bean, final int position) {
        if (mListData != null && mListData.size() != 0) {
            LogUtil.trace("待删除的内容：" + bean.toString());

            final AlertDialog.Builder normalDialog = new AlertDialog.Builder(DaojianActivity
                    .this);
            ...
            normalDialog.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    if (DaojianDBHelper.isRecordUpload(barcodeID)) {
                        Toast.makeText(DaojianActivity.this, "当前记录已上传，不能删除", Toast.LENGTH_SHORT)
                                .show();
                        return;
                    }
    ...
    private void deleteLastestRecord() {
        ...
        final CargoArrivalFileContent barcode = (CargoArrivalFileContent) mListData.get(0)
                .getScannerBean();
    ...
    private void uploadListViewDataToServer() {
        if (!NetworkUtils.isNetworkConnected(DaojianActivity
                .this)) {
            Toast.makeText(DaojianActivity.this, "网络不可用，请检查网络", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (mListData != null && mListData.size() != 0) {
            ...
            new CommonDbHelperToUploadFile<CargoArrivalFileContent>().setCallbackListener(new IDbHelperToUploadFileCallback() {

                @Override
                public boolean onSuccess(String s) {
                    // 文件上传存在延时
                    Toast.makeText(DaojianActivity.this, "文件上传成功", Toast.LENGTH_SHORT).show();
                    setHeaderRightViewText("未上传：" + searchUnloadDataForUpdate(Constant
                            .SYNC_UNLOAD_DATA_TYPE_DJ));
                    return true;
                }
