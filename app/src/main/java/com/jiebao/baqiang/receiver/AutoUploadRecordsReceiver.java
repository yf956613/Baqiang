...
public class AutoUploadRecordsReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        final BaseActivityWithTitleAndNumber mTopActivity =
                (BaseActivityWithTitleAndNumber)
        ...
        if (mTopActivity != null) {
            if (!NetworkUtils.isNetworkConnected(mTopActivity)) {
                Toast.makeText(mTopActivity, "网络不可用，请检查网络", Toast
                        .LENGTH_SHORT).show();
                return;
            }
            ...
            new CommonDbHelperToUploadFile<ZCFajianFileContent>()
                ...
                public boolean onSuccess(String s) {
                    ...
                    if (!TextUtils.isEmpty(s)) {
                        Toast.makeText(mTopActivity, s, Toast.LENGTH_SHORT)
                                .show();
                    }
                    // F1事件，传递给Activity更新UI
                    mTopActivity.syncViewAfterUpload(Constant
